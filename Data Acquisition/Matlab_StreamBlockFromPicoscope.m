...
ps2000ConfigInfo.channelSettings.channelA.range = 8;
ps2000ConfigInfo.channelSettings.channelA.range = 8;
...
prompt = {'Enter sampling interval (ms):','Enter total sampling time (s):'};
dlgtitle = 'Sampling parameters input';
fieldsize = [1 45; 1 45];
definput = {'3.2e-4','10'};
answer = inputdlg(prompt,dlgtitle,fieldsize,definput);

samplingIntervalMs = str2num(answer{1});
totalSamplingTime = str2num(answer{2});

%samplingIntervalMs = 3.2e-4; % 320 ns

set(streamingGroupObj, 'streamingIntervalMs', samplingIntervalMs);

fprintf('Sampling interval: %.6f ms.\n', samplingIntervalMs);

numStreamingSamples = (1000 * totalSamplingTime) / samplingIntervalMs;
...
totalSampleTimeTest = (samplingIntervalMs * numStreamingSamples) / 1000;

fprintf('Estimated total sampling time: %.1f s \n', totalSampleTimeTest);
...
appBufferSize = int64(numStreamingSamples * 1.5);
